...
int contactAdded(Contact *cContact) {
    ...
    int getEnvStat = g_jvm->GetEnv((void **) &globalEnv, JNI_VERSION_1_6);
    if (getEnvStat == JNI_EDETACHED) {
        LOGD("GetEnv: not attached");
        if (g_jvm->AttachCurrentThread(&globalEnv, NULL) != 0) {
    ...
    jobject jContact = getJavaObjectFromCContact(globalEnv, cContact);
    //globalEnv->CallVoidMethod(ContactsListener_Object, contact_callback_contact_added, jContact);
    globalEnv->CallStaticVoidMethod(java_com_sheshu_iacpp_ContactsSDK_ContactsSDK_class,
                                    contact_callback_contact_added, jContact);
    globalEnv->DeleteLocalRef(jContact);
...
int contactUpdated(Contact *oldContact, Contact *newContact) {
    ...
    jobject jOldContact = getJavaObjectFromCContact(globalEnv, oldContact);
    jobject jNewContact = getJavaObjectFromCContact(globalEnv, newContact);

    globalEnv->CallStaticVoidMethod(java_com_sheshu_iacpp_ContactsSDK_ContactsSDK_class,
                                    contact_callback_contact_updated, jOldContact,jNewContact);
    globalEnv->DeleteLocalRef(jNewContact);
    globalEnv->DeleteLocalRef(jOldContact);
